  ...
  useEffect(() => {
    microcmsClient
      .get({
        ...
        queries: {
          limit: 20,
          filters: `lineId[equals]${profile?.userId}`,
          orders: "-reservationAt",
        },
      })
      .then((data) => {
      ...
      {profile && (
        <header className={styles.header}>
          ...
          <p className={utilStyles.lightText}>
            こんにちは、{profile.displayName}さん、しげサロンへようこそ！
            <br />
            {previousReservations.length != 0 ? (
              <>
                来店ポイント {previousReservations.length} pt です。
                前回の来店は{" "}
                {dateToString(previousReservations[0].reservationAt)} です。
              </>
            ) : (
              ...
              {reservations.map((reservation) => (
                ...
                >
                  <Link href={`/reservations?id=${reservation.id}`}>
                    {reservation.staff?.staffName}:{" "}
                    {dateToString(reservation.reservationAt)}
                  </Link>
                </ListItem>
              ))}
            </List>
